...
public class GithubAuthorizeController {
    ...
    String callback(@RequestParam(name = "code") String code,
                    ...
                    HttpServletRequest request, // on request
                    HttpServletResponse response)   // response to client
    {
        ...
        String githubToken = getAccessToken(tg); // POST to retrieve github token
        GithubUser githubUser = getUser(githubToken);// step3
        if(githubUser != null) {
            // login success
            DB_User user = new DB_User();
            user.setToken(UUID.randomUUID().toString());    // give this login a UUID as token
    ...
    private String getAccessToken(TokenGetter getter)
    {
        ...
        String sg = JSON.toJSONString(getter);  // Object to JSON_str (amazing)
        ...
        try (Response response = client.newCall(request).execute()) {
            if(response.isSuccessful()) {
                ...
                TokenPackage tp = JSON.parseObject(s, TokenPackage.class);  // JSON_str to Object (amazing)
